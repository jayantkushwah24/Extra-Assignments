// 10. Write an ES6 function that takes an array of objects with name, role and array of
// hours which represents the time a person spends on the server each day.
// Give the tag of regular to students who are active in the community and spends more than 20 hours a week

const tagRegularStudents = (arr) => {
  // Your ES6+ code here
  const answer = arr.map((person) => {
    const totalHours = person.hours.reduce((acc, value) => acc + value, 0);
    return {
      ...person,
      ...(person.role === "student" && totalHours > 20 && { tag: "regular" }),
    };
  });
  return answer;
};
